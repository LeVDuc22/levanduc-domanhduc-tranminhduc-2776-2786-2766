...
static thread_action_func thread_calcu_recent_cpu_single;
static void thread_calcu_load_avg(void);
static heap_less_func thread_priority_cmp;
static void thread_calcu_priority(struct thread *);
...
{
    return fixed_point_to_i(fp_mul_i(load_avg, 100));
}

/* Returns 100 times the current thread's recent_cpu value, rounded to the nearest integer. */
int thread_get_recent_cpu(void)
{
    return fixed_point_to_i(fp_mul_i(thread_current()->recent_cpu, 100));
}

/* priority = PRI_MAX - (recent_cpu / 4) - (nice * 2) */
static void thread_calcu_priority(struct thread *t)
{
    ASSERT(thread_mlfqs);
    ASSERT(is_thread(t));
    if (t == idle_thread)
    ...
    t->priority = PRI_MAX - fixed_point_to_i(fp_div_i(t->recent_cpu, 4)) - t->nice * 2;
...
static void thread_calcu_recent_cpu_single(struct thread *t, void *aux UNUSED)
{
    ASSERT(is_thread(t));
    ...
    fixed_point_t k = fp_div_fp(fp_mul_i(load_avg, 2), fp_add_i(fp_mul_i(load_avg, 2), 1));
...
void thread_calcu_recent_cpu(void)
{
    thread_calcu_load_avg();
    thread_foreach(thread_calcu_recent_cpu_single, NULL);
}
...
static void thread_calcu_load_avg(void)
{
    ...
    fixed_point_t k1 = fp_div_fp(i_to_fp(59), i_to_fp(60));
    fixed_point_t k2 = fp_div_fp(i_to_fp(1), i_to_fp(60));
